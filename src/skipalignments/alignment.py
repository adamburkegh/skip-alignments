...
class State(object):
                            ...
                            for c in lca.children:
                                ...
                                if c.contains_tree(leaf):
                                    ...
                                    if new_state.state[self.mapper.node_to_index(start_node)] == NodeState.ENABLED:
                                        ...
                                        new_state.stop_execution(current_activity)
    ...
    def complete_running_subtree(self, act_running:ProcessTree, act_new:ProcessTree,lca:ProcessTree) -> "State":
        ...
        if isinstance(lca, And):
            ...
            new_state.stop_execution(act_running)
    ...
    def finish_subtree(self, root:ProcessTree, act_running:ProcessTree):
        ...
        new_state = self.copy()
        new_state.state[self.mapper.node_to_index(act_running)] = NodeState.CLOSED
        #print("Closing running subtree with activity:", act_running, new_state.path)
        new_state.stop_execution(act_running)
    ...
    def complete_partial_tree(self, tree:ProcessTree, state:"State"):
        ...
        if isinstance(tree, LeafNode):
            if state.state[self.mapper.node_to_index(tree)] == NodeState.ENABLED:
                ...
                #print("Closing in complete partial tree:", tree, state.path)
                state.stop_execution(tree)
            ...
            if state.state[self.mapper.node_to_index(tree)] == NodeState.ENABLED:
                ...
                if state.state[self.mapper.node_to_index(tree.children[0])] == NodeState.ACTIVE:
                    ...
                    state.stop_execution(tree.children[0])
    ...
    def get_shortest_path_down(self, start_node:ProcessTree, leaf_node:ProcessTree, new_state:"State"=None):
        ...
        while len(reverse_node_path) > 0:
            ...
            if isinstance(current_node, Sequence):
                new_state.start_execution(current_node)
    ...
    def _shortest_execution(self, tree:ProcessTree, new_state:"State"):
        ...
        if isinstance(tree, LeafNode):
            ...
            new_state.stop_execution(tree)
            #print("Closing activity in _shortest_execution:", tree, new_state.path)
            return [('>>', tree)], [tree]
        agn_path = []
        fired_leafs = []
    ...
    def start_execution(self, node:ProcessTree):
        for i in range(len(self.executions)-1, -1, -1):
            if self.executions[i].node is not None and self.executions[i].node == node and self.executions[i].start is not None and self.executions[i].stop is None:
                # execution of node is already started and ongoing, skip
                return
        self.executions.append(Execution(node, len(self.path)))
    
    def stop_execution(self, node:ProcessTree):
        for i in range(len(self.executions)-1, -1, -1):
            # if execution of node is already stopped, then skip
            if self.executions[i].node is not None and self.executions[i].node == node and self.executions[i].start is not None and self.executions[i].stop is None:
                self.executions[i].stop = len(self.path)
                return
    ...
    def unfold(self):
        ...
        for i, (_, model_move) in enumerate(self.path):
            if isinstance(model_move, TauPath) or isinstance(model_move, Skip):
        ...
        for path in list(itertools.product(*paths)):
            # path = [([acts1], [execs1]), ([acts2], [execs2]), ...] for skips 1, 2, ...
            new_state = self.copy()
            i = 0
            while i < len(new_state.path):
                # merge in by replacing skip at pos i
                if new_state.path[i][0] != '>>': # no skip move
                    i += 1
                    continue
                #print("Dealing with move", new_state.path[i], "at pos", i)
                path_before = new_state.path[:i]
                ...
                for j in range(len(new_state.executions)):
                    if new_state.executions[j].start < i and new_state.executions[j].stop > i:
                        ...
                        #print("Updating", new_state.executions[j], "on front&end by", len(path[0][0]))
                        new_state.executions[j].stop += len(path[0][0])-1
                        new_state.executions[j].start += len(path[0][0])-1
                new_state.executions += [Execution(exec.node, exec.start+len(path_before), exec.stop+len(path_before)) for exec in path[0][1]]
                i += len(path[0][0])
                path = path[1:]
    ...
    def copy(self):
        ...
        for exec in self.executions:
            state.executions.append(Execution(exec.node, exec.start, exec.stop))
        return state
