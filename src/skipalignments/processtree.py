...
class ProcessTree(object):
    ...
    def to_pm4py(self, use_ids=False):
        """
        Translates the current process tree into a representation for pm4py.
        """
        if isinstance(self, Tau):
            # tau
            return pm4py.objects.process_tree.obj.ProcessTree(None, None, None, "TAU_" + self.name if not use_ids else self.id)
        if isinstance(self, Activity):
            # activity
            return pm4py.objects.process_tree.obj.ProcessTree(None, None, None, self.name if not use_ids else self.id)
        assert not isinstance(self, LeafNode)
        children = [c.to_pm4py(use_ids) for c in self.children]
...
class Skip(ProcessTree):
    ...
    def unfold(self):
        if isinstance(self.node, Tau) or isinstance(self.node, Activity):
            return [([self.node], [Execution(self.node, 0, 1)])]
        elif isinstance(self.node, Sequence) or isinstance(self.node, And): # we fix the order of And executions
            paths = []
            for c in self.node.children:
                paths.append(Skip(c, c.get_cheapest_execution(0)[0]).unfold())
            res = []
            for path in list(itertools.product(*paths)):
                # path = [([c1 acts], [c1 execs]), ([c2 acts], [c2 execs])]
                trace = [t for elem in path for t in elem[0]]
                prefix_length = 0
                executions = []
                for elem in path:
                    for t in elem[1]:
                        executions.append(Execution(t.node, t.start+prefix_length, t.stop+prefix_length))
                    prefix_length += len(elem[0])
                res.append((trace, executions + [Execution(self.node, 0, len(trace))]))
            return res
        elif isinstance(self.node, Xor):
            # we don't know yet which path is cheapest
            paths = []
            min_cost = self.node.get_cheapest_execution(0)[0]
            for c in self.node.children:
                if c.get_cheapest_execution(0)[0] == min_cost:
                    intermediate = Skip(c, c.get_cheapest_execution(0)[0]).unfold()
                    # intermediate = [([c1 acts], [c1 execs]), ([c1' acts], [c1' execs])]
                    paths += [(elem[0], elem[1]+[Execution(self.node, 0, len(elem[0]))]) for elem in intermediate]
            return paths
        elif isinstance(self.node, Loop):
            intermediate = Skip(self.node.children[0], self.node.children[0].get_cheapest_execution(0)[0]).unfold()
            return [(elem[0], elem[1]+[Execution(self.node, 0, len(elem[0]))]) for elem in intermediate]
...
class TauPath(ProcessTree):
    ...
    def unfold(self):
        return Skip(self.node, 0).unfold()
...
class Execution(object):
    def __init__(self, node=None, start=None, stop=None):
        self.node:ProcessTree = node
        self.start:int = start
        self.stop:int = stop
    
    def __str__(self) -> str:
        return "Exec( " + self.node.__str__() + ", " + str(self.start) + ", " + str(self.stop) + " )"
