"""
One-off diagnostic (not a unit test) reproducing a process-voids report:
build_execution_tree's `assert tree_c is not None` (execution.py:338)
firing on a real inductive_noise20-discovered rtfm.xes tree, activity_gradual
degradation level=0.6, several trace variants containing
'Receive Result Appeal from Prefecture' -- an activity with no leaf
anywhere in this tree's alphabet at all (degradation left a stray event
behind after removing the rest of that activity's occurrences).

Tries each reported trace variant against the tree directly through
Aligner.align_normal_form + ExecutionManager.coninciding_agns (skipping
DerivationPipeline/Ebi entirely -- coninciding_agns runs before Ebi is
ever invoked in DerivationPipeline.compute, so it isn't needed to
reproduce this), to find which variant(s) actually trigger it and narrow
to the smallest one for a permanent regression test.

Run with:
    skip/Scripts/python.exe tests/rtfm_coninciding_agns_diagnostic.py
"""
from skipalignments.alignment import Aligner
from skipalignments.execution import ExecutionManager
from skipalignments.processtree import Activity, And, Sequence, Tau, Xor

COST = 100000

VARIANTS = [
    ('Send Fine', 'Add penalty'),
    ('Send Fine',),
    ('Add penalty',),
    ('Send Fine', 'Add penalty', 'Receive Result Appeal from Prefecture'),
    ('Add penalty', 'Receive Result Appeal from Prefecture'),
    ('Send Fine', 'Receive Result Appeal from Prefecture', 'Add penalty'),
    ('Receive Result Appeal from Prefecture', 'Add penalty'),
    ('Receive Result Appeal from Prefecture',),
]


def build_tree():
    def act(name):
        a = Activity(None, name, COST)
        a.id = name
        return a

    def tau(name):
        t = Tau(None, name, 0)
        t.id = name
        return t

    insert_date = act('Insert Date Appeal to Prefecture')
    tau1 = tau('tau1')
    xor1 = Xor(None, [tau1, insert_date])
    tau1.set_parent(xor1)
    insert_date.set_parent(xor1)
    xor1.id = 'xor1'

    appeal_judge = act('Appeal to Judge')
    send_fine = act('Send Fine')
    tau2 = tau('tau2')
    insert_notif = act('Insert Fine Notification')
    xor_notif = Xor(None, [tau2, insert_notif])
    tau2.set_parent(xor_notif)
    insert_notif.set_parent(xor_notif)
    xor_notif.id = 'xor_notif'

    seq_send_notif = Sequence(None, [send_fine, xor_notif])
    send_fine.set_parent(seq_send_notif)
    xor_notif.set_parent(seq_send_notif)
    seq_send_notif.id = 'seq_send_notif'

    xor_judge_or_seq = Xor(None, [appeal_judge, seq_send_notif])
    appeal_judge.set_parent(xor_judge_or_seq)
    seq_send_notif.set_parent(xor_judge_or_seq)
    xor_judge_or_seq.id = 'xor_judge_or_seq'

    tau3 = tau('tau3')
    add_penalty = act('Add penalty')
    xor_penalty = Xor(None, [tau3, add_penalty])
    tau3.set_parent(xor_penalty)
    add_penalty.set_parent(xor_penalty)
    xor_penalty.id = 'xor_penalty'

    seq_inner = Sequence(None, [xor_judge_or_seq, xor_penalty])
    xor_judge_or_seq.set_parent(seq_inner)
    xor_penalty.set_parent(seq_inner)
    seq_inner.id = 'seq_inner'

    and_node = And(None, [xor1, seq_inner])
    xor1.set_parent(and_node)
    seq_inner.set_parent(and_node)
    and_node.id = 'and_node'

    tau_root = tau('tau_root')
    root = Xor(None, [tau_root, and_node])
    tau_root.set_parent(root)
    and_node.set_parent(root)
    root.id = 'root'

    return root


def main():
    tree = build_tree()
    Aligner.set_level_incentive(0)
    em = ExecutionManager()

    for variant in VARIANTS:
        aligner = Aligner(tree)
        try:
            states, _ = aligner.align_normal_form(list(variant), [COST] * len(variant), all_optimal=True, timeout=30)
        except Exception as e:
            print(f"{variant}: align_normal_form itself raised {type(e).__name__}: {e}")
            continue
        if not states:
            print(f"{variant}: no optimal states found (timeout or infeasible)")
            continue
        try:
            em.coninciding_agns({','.join(variant): states})
            print(f"{variant}: OK ({len(states)} states)")
        except AssertionError as e:
            print(f"{variant}: REPRODUCED AssertionError ({len(states)} states)")
        except Exception as e:
            print(f"{variant}: other exception {type(e).__name__}: {e}")


if __name__ == '__main__':
    main()
